...
TILE = 16   # each tile is TILE×TILE pixels (power of 2: 4, 8, 16, 32)
            # 16 = good balance of speed vs. resolution

def process_large_image(img_gray: Image.Image, encoder_cls, tile_size=TILE):
    """
    Split a grayscale PIL image into tile_size×tile_size patches,
    run quantum edge detection on each patch, stitch back together.
    Returns (edge_map, h_map, v_map) as numpy arrays matching img_gray size.
    """
    img_arr = numpy.array(img_gray, dtype=float) / 255.0
    H, W = img_arr.shape

    # Pad so dimensions are exact multiples of tile_size
    ...
    for row in range(0, PH, tile_size):
        for col in range(0, PW, tile_size):
            ...
            try:
                ...
                pass  # leave tile black on failure
            done += 1
...
def thin_edges(edge_img: numpy.ndarray) -> numpy.ndarray:
    """Skeletonize a binary/float edge map to get 1-pixel-wide lines."""
...
# Load image
# ---------------------------------------------------------------------------
IMAGE_PATH = r"your_image.jpg"   # <-- change this to your image path
...
try:
    ...
    axes[0].imshow(img_color);  axes[0].set_title("Original (colour)"); axes[0].axis("off")
    ...
    print(f"[WARN] '{IMAGE_PATH}' not found — using built-in 4×4 demo pattern.")
    arr = numpy.array([
        [0, 0, 0, 0],
        [0, 1, 1, 0],
        [0, 1, 1, 0],
...
if USE_TILING:
    print(f"[QPIE] Tiled mode — tile size: {TILE}×{TILE}")
    ...
    plt.title("QPIE Encoding Circuit"); plt.tight_layout(); plt.show()
    try:
...
if qpie_edge_img is not None:
    ...
    axes[1].imshow(h_img, cmap='gray');        axes[1].set_title('H-Edges (QPIE)')
    axes[2].imshow(v_img, cmap='gray');        axes[2].set_title('V-Edges (QPIE)')
    axes[3].imshow(qpie_edge_img, cmap='gray');axes[3].set_title('Combined Edges (QPIE)')
...
if USE_TILING:
    print(f"[FRQI] Tiled mode — tile size: {TILE}×{TILE}")
    ...
    plt.title("FRQI Encoding Circuit"); plt.tight_layout(); plt.show()
    try:
...
if frqi_edge_img is not None:
    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    axes[0].imshow(arr, cmap='gray');           axes[0].set_title('Original')
    axes[1].imshow(h_img_f, cmap='gray');       axes[1].set_title('H-Edges (FRQI)')
    axes[2].imshow(v_img_f, cmap='gray');       axes[2].set_title('V-Edges (FRQI)')
    axes[3].imshow(frqi_edge_img, cmap='gray'); axes[3].set_title('Combined Edges (FRQI)')
    for ax in axes: ax.axis('off')
